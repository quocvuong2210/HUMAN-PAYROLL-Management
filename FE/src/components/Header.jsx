...
export default function Header({ isDarkMode, toggleDarkMode, language, toggleLanguage }) {
  ...
  const displayName = userData?.fullname || userData?.FullName || userData?.username || userData?.Username || (isVi ? 'Người dùng' : 'User');
  const userRole = userData?.roles?.[0]?.RoleName || userData?.Roles?.[0]?.RoleName || userData?.roles?.[0] || userData?.Roles?.[0] || 'USER';

  console.log('👤 Display Name:', displayName);
  console.log('🎭 User Role:', userRole);

  return (
    <div className={`h-16 flex items-center justify-between px-6 shadow-sm border-b transition-all duration-300 z-50 relative ${isDarkMode ? 'bg-[#1e1e2d] border-slate-800' : 'bg-white border-slate-200'
      }`}>
      ...
      <div className="flex items-center gap-1 z-10">
        <div
          ...
          className={`flex items-center gap-2 px-3 py-1.5 rounded-xl cursor-pointer transition-all border border-transparent ${isDarkMode ? 'hover:bg-slate-800 hover:border-slate-700 text-slate-300' : 'hover:bg-slate-50 hover:border-slate-100 text-slate-600'
            }`}
        >
          <img
        ...
        <button
          ...
          className={`p-2.5 rounded-xl transition-all duration-500 ${isDarkMode ? 'bg-slate-800 text-yellow-400 border border-slate-700' : 'bg-slate-100 text-slate-600 border border-slate-200'
            }`}
        >
