      ...
      {error ? (
        ...
        <div className="flex-1 flex flex-col gap-4 min-h-0 overflow-hidden animate-in fade-in duration-500">
          ...
          <div className="flex items-center justify-between flex-shrink-0">
            <div className={`flex items-center rounded-2xl p-1.5 shadow-md transition-all border ${isDarkMode ? 'bg-slate-950 border-slate-700' : 'bg-slate-50 border-slate-100'
              }`}>
              ...
              <div className="flex items-center px-3 gap-2 group">
                ...
                <select
                  className={`bg-transparent border-none text-xs font-black focus:ring-0 cursor-pointer p-0 ${isDarkMode ? 'text-slate-200' : 'text-slate-700'
                    }`}
                  value={selectedMonth}
          ...
          <div className="grid grid-cols-2 gap-4 flex-shrink-0 h-[350px] min-h-0">
            ...
            <div className={`p-5 flex flex-col overflow-hidden transition-all duration-300 shadow-sm rounded-xl border ${isDarkMode ? 'bg-slate-950 border-slate-700' : 'bg-slate-50 border-slate-100'
              }`}>
              ...
              <div className="flex-1 min-h-0 relative">
                {chartsData?.bar_chart && chartsData.bar_chart.length > 0 ? (
                  <Bar
                    key={`bar-${language}`}
                    options={{
                      responsive: true,
            ...
            <div className={`p-5 flex flex-col overflow-hidden transition-all duration-300 shadow-sm rounded-xl border ${isDarkMode ? 'bg-slate-950 border-slate-700' : 'bg-slate-50 border-slate-100'
              ...
              <div className="flex-1 min-h-0 relative">
                {chartsData?.line_chart && chartsData.line_chart.length > 0 ? (
                  <Line
                    key={`line-${language}`}
                    options={{
                      responsive: true,
          ...
          <div className="grid grid-cols-12 gap-4 h-[300px] flex-shrink-0 mb-4">
            ...
            <div className={`col-span-4 p-5 flex flex-col overflow-hidden transition-all duration-300 shadow-sm rounded-xl border ${isDarkMode ? 'bg-slate-950 border-slate-700' : 'bg-slate-50 border-slate-100'
              }`}>
              ...
              <div className="flex-1 relative flex items-center justify-center min-h-0">
                {chartsData?.pie_chart && chartsData.pie_chart.length > 0 ? (
                  <>
                    <Doughnut
                      key={`pie-${language}`}
                      options={{
                    ...
                    <div className="absolute top-[40%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center pointer-events-none">
                      <span className={`text-2xl font-black leading-none ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
                        {chartsData.pie_chart.reduce((acc, curr) => acc + curr.value, 0)}
                      </span>
            ...
            <div className={`col-span-8 p-5 flex flex-col overflow-hidden transition-all duration-300 shadow-sm rounded-xl border ${isDarkMode ? 'bg-slate-950 border-slate-700' : 'bg-slate-50 border-slate-100'
              }`}>
