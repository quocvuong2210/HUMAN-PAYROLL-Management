...
import {
  ...
  Users, BarChart3, PieChart as PieChartIcon, RefreshCw
} from 'lucide-react';
...
export default function EmployeesPage() {
  ...
  const t = {
    vi: {
      ...
      stats: {
        byDept: "Nhân Viên Theo Phòng Ban",
        byPosition: "Nhân Viên Theo Chức Vụ",
        byStatus: "Phân Bổ Theo Trạng Thái",
        byGender: "Phân Bổ Theo Giới Tính",
    ...
    en: {
      ...
      stats: {
        byDept: "Employees by Department",
        byPosition: "Employees by Position",
        byStatus: "Distribution by Status",
        byGender: "Distribution by Gender",
  ...
  const handleOpenForm = (emp = null) => {
    if (emp) {
      console.log('📝 Opening edit form for employee:', emp);
      console.log('📅 DateOfBirth from API:', emp.DateOfBirth);
      console.log('📅 HireDate from API:', emp.HireDate);
      ...
      console.log('✅ Formatted HireDate:', formattedData.HireDate);
  ...
  const statsByDept = departments.map(dept => ({
    ...
    value: employees.filter(emp => emp.DepartmentID === dept.DepartmentID).length
  })).filter(item => item.value > 0);

  const statsByPosition = positions.map(pos => ({
    name: pos.PositionName,
    value: employees.filter(emp => emp.PositionID === pos.PositionID).length
  })).filter(item => item.value > 0);

  const statsByStatus = [
    { name: 'Đang làm việc', value: employees.filter(e => e.Status === 'Đang làm việc').length, color: '#10b981' },
    { name: 'Nghỉ phép', value: employees.filter(e => e.Status === 'Nghỉ phép').length, color: '#f59e0b' },
    { name: 'Thử việc', value: employees.filter(e => e.Status === 'Thử việc').length, color: '#3b82f6' },
    { name: 'Thực tập', value: employees.filter(e => e.Status === 'Thực tập').length, color: '#8b5cf6' }
  ].filter(item => item.value > 0);

  const statsByGender = [
    { name: 'Nam', value: employees.filter(e => e.Gender === 'Nam').length, color: '#3b82f6' },
    { name: 'Nữ', value: employees.filter(e => e.Gender === 'Nữ').length, color: '#ec4899' },
    { name: 'Khác', value: employees.filter(e => e.Gender === 'Khác').length, color: '#8b5cf6' }
  ].filter(item => item.value > 0);

  const tabs = [
    { id: 'list', label: t.listTab, icon: Users },
      ...
      <div className="flex items-center justify-between border-b border-gray-200 dark:border-slate-700 pb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {activeTab === 'list' && (
            ...
            >
              <UserPlus size={16} /> {t.addBtn}
            </button>
          )}
        </div>
      </div>

      {/* SEARCH & FILTER BAR - Only show on list tab */}
      {activeTab === 'list' && (
        <div className={`p-4 border space-y-4 ${isDarkMode ? 'bg-slate-900/40 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                  ...
                  {loading ? (
                    ...
                    employees.map((emp) => (
                      <tr key={emp.EmployeeID} className="hover:bg-blue-600/5 transition-all group">
                        ...
                        <td className="px-6 py-2">
                          <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold ${emp.Status === 'Đang làm việc' ? 'bg-green-500/10 text-green-500' :
                            emp.Status === 'Thử việc' ? 'bg-blue-500/10 text-blue-500' :
                              emp.Status === 'Thực tập' ? 'bg-purple-500/10 text-purple-500' :
      ...
      {/* STATISTICS TAB */}
      {
        activeTab === 'stats' && !loading && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* By Department */}
            <div className={`p-5 rounded-xl border ${isDarkMode ? 'bg-slate-900/40 border-slate-800' : 'bg-white border-slate-200'}`}>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={statsByDept}>
                    ...
                    <Bar dataKey="value" name="Nhân viên" fill="#3b82f6" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    >
                      {statsByPosition.map((entry, index) => (
            ...
            <div className="lg:col-span-2 grid grid-cols-4 gap-4">
              <div className={`p-5 rounded-xl border ${isDarkMode ? 'bg-slate-900/40 border-slate-800' : 'bg-white border-slate-200'}`}>
                <p className="text-xs font-bold text-blue-500 uppercase mb-2">{t.stats.total}</p>
                <p className={`text-3xl font-black ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{employees.length}</p>
              </div>
              <div className={`p-5 rounded-xl border ${isDarkMode ? 'bg-slate-900/40 border-slate-800' : 'bg-white border-slate-200'}`}>
                <p className="text-xs font-bold text-emerald-500 uppercase mb-2">{t.stats.active}</p>
                <p className={`text-3xl font-black ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
                  {employees.filter(e => e.Status === 'Đang làm việc').length}
                </p>
              </div>
              <div className={`p-5 rounded-xl border ${isDarkMode ? 'bg-slate-900/40 border-slate-800' : 'bg-white border-slate-200'}`}>
                <p className="text-xs font-bold text-amber-500 uppercase mb-2">{t.stats.trial}</p>
                <p className={`text-3xl font-black ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
                  {employees.filter(e => e.Status === 'Thử việc').length}
                </p>
              </div>
              <div className={`p-5 rounded-xl border ${isDarkMode ? 'bg-slate-900/40 border-slate-800' : 'bg-white border-slate-200'}`}>
                <p className="text-xs font-bold text-purple-500 uppercase mb-2">Thực Tập</p>
                <p className={`text-3xl font-black ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
                  {employees.filter(e => e.Status === 'Thực tập').length}
                </p>
              </div>
            </div>
          </div>
