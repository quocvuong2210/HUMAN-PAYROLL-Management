...
export default function HRManagementPage() {
  ...
  const exportToExcel = () => {
  // Gọi trực tiếp đến API export đã xây dựng
  // Truyền thêm tháng đang chọn để server biết xuất dữ liệu tháng nào
  window.location.href = `${API_BASE}/api/v1/salary/export?month=${month}`;
};

  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text(`HR Report: ${activeTab.toUpperCase()} - ${month}`, 14, 15);
    const tableColumn = t.cols[activeTab].filter(c => c !== '');
    const tableRows = data.map(i => Object.values(i).slice(0, tableColumn.length));
    doc.autoTable({ head: [tableColumn], body: tableRows, startY: 20 });
    doc.save(`HR_Report_${activeTab}_${month}.pdf`);
  };
  ...
  <div className="flex gap-2">
            <button onClick={exportToExcel} className="text-xs px-3 py-1.5 bg-emerald-600/10 text-emerald-600 rounded-lg flex items-center gap-2 hover:bg-emerald-600/20"><FileDown size={14}/> Excel</button>
